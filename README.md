...
The system focuses on **per-query optimization** and is compatible with both **single-query** and **batch-processing** pipelines.
...
These models are guaranteed to support generateContent.
...
Prompt optimization, model selection, and execution are independent modules.

Cost-First Design
Expensive models are used only when required.

Extensible Architecture
Easy to add new models, rules, or logging backends.
...
Summary
LLM Optimizer provides a clean, production-ready foundation for building efficient, cost-aware LLM systems, ensuring each query is handled by the right model at the right cost.
